...
const roles = [
  {
    ...
    description: "Auto-delete mode: Suspicious messages are automatically deleted. They never see scam attempts.",
  },
  {
    value: "child",
    label: "Child",
    ...
    description: "Strict protection: Only whitelisted contacts can send messages. All unknown senders are blocked.",
  },
  {
    value: "parent",
    label: "Parent/Admin",
    ...
    description: "Review mode: Suspicious messages are quarantined for review. Full control over family protection.",
  },
];
...
export default function AddMember() {
  ...
  const [trustedContacts, setTrustedContacts] = useState<string[]>(["+91-9876543210", "+91-1234567890"]);
                ...
                {roles.map((role) => (
                  <div
                    ...
                    className={`flex items-start space-x-3 p-4 rounded-lg border-2 transition-colors cursor-pointer ${
                      selectedRole === role.value ? role.bgColor : "border-border hover:bg-muted/50"
                    }`}
          ...
          <CardContent className="space-y-6">
                ...
                {protectionModes.map((mode) => (
                  <div
                    ...
                    className={`flex items-start space-x-3 p-3 rounded-lg border transition-colors cursor-pointer ${
                      protectionMode === mode.value ? "border-primary bg-primary/5" : "border-border hover:bg-muted/50"
                    }`}
                    onClick={() => setProtectionMode(mode.value)}
                  >
                    <RadioGroupItem value={mode.value} id={`mode-${mode.value}`} className="mt-0.5" />
                    <div>
                      <Label htmlFor={`mode-${mode.value}`} className="font-medium cursor-pointer">
                        {mode.label}
                      </Label>
            ...
            <div className="flex items-center justify-between p-4 rounded-lg bg-muted/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Speak alerts for elderly members</p>
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <p className="text-sm text-muted-foreground">
                Messages from these contacts will never be blocked
              </p>
              ...
              <div className="flex gap-2">
                ...
                />
                <Button onClick={addTrustedContact} variant="outline" size="icon">
                ...
                {trustedContacts.map((contact) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="ml-1 h-4 w-4 rounded-full bg-muted-foreground/20 hover:bg-destructive hover:text-destructive-foreground flex items-center justify-center transition-colors"
                    >
                      <X className="h-3 w-3" />
                    </button>
                  </Badge>
